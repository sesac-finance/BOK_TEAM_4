...
def cleansing(text):
    ...
    pattern = '<[^>]*>' # html 제거
    text = re.sub(pattern = pattern, repl=repl,string=text)
    pattern = '\.([^\.]*(?:기자|뉴스|특파원|교수|작가|대표|논설|고문|주필|부문장|팀장|장관|원장|연구원|이사장|위원|실장|차장|부장|에세이|화백|사설|소장|단장|과장|기획자|큐레이터|저작권|평론가|©|©|ⓒ|\@|\/|=|:앞쪽_화살표:|무단|전재|재배포|금지|\[|\]|\(\))[^\.]*)$'
    text = re.sub(pattern = pattern, repl=repl,string=text)
    return text
...
if __name__ == "__main__":
    df=news_ngram().to_csv('news_ngram_data.csv')
